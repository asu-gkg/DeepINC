    ...
    namespace common
    {
        namespace compressor
        {
            ...
            void EliasDeltaEncode(BitWriter<T> &bit_writer, unsigned long x)
            {
                ...
                for (int i = lenth_of_len; i > 0; --i)
                    bit_writer.Put(0);
                for (int i = lenth_of_len; i >= 0; --i)
                    bit_writer.Put((len >> i) & 1);
                for (int i = len - 2; i >= 0; i--)
                    bit_writer.Put((x >> i) & 1);
            }
            ...
            T HyperParamFinder(const kwargs_t &kwargs, std::string name, bool optional = false, F &&check = [](T)
                                                                                                { return true; })
            {
